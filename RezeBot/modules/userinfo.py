...
from RezeBot.__main__ import STATS, TOKEN, USER_INFO
from RezeBot.modules.sql import SESSION
import RezeBot.modules.sql.userinfo_sql as sql
from RezeBot.modules.disable import DisableAbleCommandHandler
from RezeBot.modules.sql.global_bans_sql import is_user_gbanned
from RezeBot.modules.sql.afk_sql import is_afk, set_afk
from RezeBot.modules.sql.users_sql import get_user_num_chats
from RezeBot.modules.helper_funcs.chat_status import sudo_plus
from RezeBot.modules.helper_funcs.extraction import extract_user
from RezeBot import telethn
...
def info(update: Update, context: CallbackContext):
    ...
    rep = message.reply_text("<code>Contacting Core For Info.....</code>", parse_mode=ParseMode.HTML)
    ...
    if user.id == OWNER_ID:
        ...
        text += "\n\nThis user is a B Class Devil."
        disaster_level_present = True
    elif user.id in TIGERS:
        text += "\n\nThis user is a C Class Devil."
        disaster_level_present = True
    elif user.id in WOLVES:
        text += "\n\nThis user is a Normal Devil'."
        disaster_level_present = True
...
def stats(update, context):
    ...
    try:
        update.effective_message.reply_text(
            ...
            + f"\n\n✦ [sᴜᴘᴘᴏʀᴛ](https://t.me/Zeke_grp) | ✦ [ᴜᴘᴅᴀᴛᴇs](https://t.me/Reze_updates)\n\n"
            + "╘══「 By [ʀᴇᴢᴇ](https://t.me/REZE_sUPER_BOT) | [sᴇɴsᴇɪ](https://t.me/Physco_Sensei) 」\n",
            parse_mode=ParseMode.MARKDOWN,
            disable_web_page_preview=True,
        )
        ...
        update.effective_message.reply_text(
            (
                (
                    ...
                    + f"\n\n✦ [Support](https://t.me/Zeke_grp) | ✦ [Updates](https://t.me/Reze_updates)\n\n"
                )
                + "╘══「 By [Reze](https://t.me/REZE_sUPER_BOT) | [Sensei](https://t.me/Physco_Sensei)」\n"
            ),
            parse_mode=ParseMode.MARKDOWN,
            disable_web_page_preview=True,
        )
...
def set_about_bio(update: Update, context: CallbackContext):
    ...
    if message.reply_to_message:
        ...
        if user_id == bot.id and sender_id not in DEV_USERS:
            message.reply_text(
                "Erm... yeah, I only My Owner to set my bio.",
            )
            return
