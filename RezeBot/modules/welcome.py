...
import RezeBot.modules.sql.welcome_sql as sql
from RezeBot import (
    DEV_USERS,
    OWNER_ID,
    DRAGONS,
...
from RezeBot.modules.helper_funcs.misc import build_keyboard, revert_buttons
from RezeBot.modules.helper_funcs.msg_types import get_welcome_type
from RezeBot.modules.helper_funcs.handlers import MessageHandlerChecker
from RezeBot.modules.helper_funcs.string_handling import (
    escape_invalid_curly_brackets,
    markdown_parser,
)
from RezeBot.modules.log_channel import loggable
...
PHYWELCOME = 'https://telegra.ph/file/f7a8b2408a2e973bbbfa7.jpg'
        ...
        if should_welc:
            ...
            if new_mem.id == OWNER_ID:
                update.effective_message.reply_text(
                   PHYWELCOME, f"Welcome to {html.escape(chat.title)} my king.", reply_to_message_id=reply
                )
                welcome_log = (
                    f"{html.escape(chat.title)}\n"
                    f"#USER_JOINED\n"
                    f"My Sensei just joined the chat"
                )
                continue
            ...
            if new_mem.id in DEV_USERS:
                update.effective_message.reply_text(
                    "Whoa! The S class devil just joined!",
                    reply_to_message_id=reply,
                )
                continue
            ...
            if new_mem.id in DRAGONS:
                update.effective_message.reply_text(
                    "Huh! A class devil just joined! Stay Alert!",
                    reply_to_message_id=reply,
                )
                continue
            ...
            if new_mem.id in DEMONS:
                update.effective_message.reply_text(
                    "Huh! Someone with A class devil just joined!",
                    reply_to_message_id=reply,
                )
                continue
            ...
            if new_mem.id in WOLVES:
                update.effective_message.reply_text(
                    "Oof! One of a B Class devil Users just joined!", reply_to_message_id=reply
                )
                continue
